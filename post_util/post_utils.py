...
class plot3dClass( object ):
    ...
    def drawCenters(self, myCenters, h):
        ...
        if non_branch_points.any():
            # print('non_branch_points')
            pass
            # make_plot(non_branch_points)
...
def make_plot(plot_points,colors = False, point_size = 0.0005, compare_points=None):
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(plot_points)
    pcd.paint_uniform_color([0,0,0])
    o3d.visualization.draw_geometries([pcd], "骨架后")
    if compare_points is not None:
        pcd_compare = o3d.geometry.PointCloud()
        pcd_compare.points = o3d.utility.Vector3dVector(compare_points)
        pcd_compare.paint_uniform_color([1, 0, 0])
        o3d.visualization.draw_geometries([pcd_compare])
        o3d.visualization.draw_geometries([pcd, pcd_compare])
