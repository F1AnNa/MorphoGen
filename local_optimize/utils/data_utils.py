...
def my_collate(data):
    ...
    for arr in data:
        ...
        first_part = arr[0:1]
        second_part = arr[1:2]
        # 将拆分后的前半部分添加到 first_parts 列表
        first_parts.append(first_part)
        # 将拆分后的后半部分添加到 second_parts 列表
        second_parts.append(second_part)

    # 使用 np.concatenate 函数沿着第一个维度拼接前半部分
    result1 = np.concatenate(first_parts, axis=0)
    # 使用 np.concatenate 函数沿着第一个维度拼接后半部分
    result2 = np.concatenate(second_parts, axis=0)
...
class ConditionalPrefixSeqDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if target_l.shape != torch.Size([32, 3]):
            print('warning2')
