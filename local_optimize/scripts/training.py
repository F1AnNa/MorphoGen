...
def train_conditional_one_epoch(
    ...
    for data in tqdm(dataloader):
        ...
        noise_branch , smooth_branch = torch.from_numpy(noise_branch).to(device) , torch.from_numpy(smooth_branch).to(device)

        denoise_branch = model(noise_branch,smooth_branch,bs,teaching)
        loss1 = recon_loss(denoise_branch, smooth_branch)
        optimizer.zero_grad()
        loss1.backward()
        optimizer.step()
