...
def measure(output_dir, data_dir, split_json, is_morph=False):
    ...
    ############### data load ###############
    neurons, neuron_file = load_neurons(data_dir, scaling=0.01, return_filelist=True)
    ...
    mean_record = {
            ...
            "MBPL":Branch_pathlength_sum/l,
            "MMED":EucDistance_sum/l,
            "MMPD":PathDistance_sum/l,
            "MCTT":Contraction_sum/l,
...
def run(info, measure_name="measure", is_morph=False):
    ...
    if measure_name == "measure_forest":
        for output_dir,data_dir,split_json in zip(output_dirs,data_dirs,split_jsons):
            result.append(measure_fun(output_dir, data_dir, split_json))
