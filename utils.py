...
def farthest_point_sample_faster(points,n):
    # visual(points)
    pts = points
    num_points = pts.shape[0]

    distance = np.ones(num_points) * 1e10
    farthest_idx = np.random.randint(0, num_points)
    sampled_indices = [farthest_idx]

    for _ in range(n - 1):
        dist = np.sum((pts - pts[farthest_idx]) ** 2, axis=1)
        distance = np.minimum(distance, dist)
        farthest_idx = np.argmax(distance)
        sampled_indices.append(farthest_idx)
    real_indices = np.sort(sampled_indices)
    sampled_pts = pts[real_indices]
    # visual(sampled_pts)
    return sampled_pts

def resample_branch_by_length(branch, length, seq_len=None, eps=1e-7):
    if seq_len is None:
        seq_len = len(branch)
    ...
    for i in range(1, seq_len):
        ...
        while tres >= res - eps and len(answer) < 32:
        # while tres >= res - eps:
...
def resample_branch_by_step(branch, step, seq_len, eps=1e-5):
    # print('no process branch:',branch)
    if branch.shape[0] <= step:
        total_length = sum([
            euclidean_dist(branch[i - 1], branch[i])
            for i in range(1, seq_len)
        ])
        new_branch = resample_branch_by_length(
            branch, total_length / (step - 1), seq_len, eps
        )
        # print('processed branch:', new_branch)
        return new_branch
    else:
        # visual(branch)
        new_branch = farthest_point_sample_faster(branch,step)
        # print('process branch:', new_branch)
        # visual(point)
        return new_branch
...
class Tree(object):
    ...
    def easy_fetch_resample(self,align, move=True):
        branches = []
        branch_branch = {}
        branch_last  = {}
        node_branch = {}
        ...
        for i in range(len(self.nodes)):
            ...
            # if len(branches) == 323:
            #     test = branches[322]
            if node.father==None:#胞体

                node_branch.update({node.Idx:-1})
                continue
            elif node.father.father==None:#6：创

                branches.append(np.stack((node.father.data['pos'],node.data['pos']),axis=0))
                # branch_last.update({node.Idx:bn})
                # branch_start.update({bn:-1})
                node_branch.update({node.Idx:bn})
                branch_branch.update({bn:-1})
                bn = bn+1
            elif len(node.father.sons)==1:#2,3,5：加
                fa_node = node.father.Idx
                # if fa_node == 1080:
                #     n = 1
                fa_branch = node_branch[fa_node]
                branches[fa_branch]=np.vstack((branches[fa_branch],node.data['pos']))
                # branch_last.update({node.Idx:bn})
                node_branch.update({node.Idx:fa_branch})

            elif len(node.father.sons)>1:#4：创
                if np.array_equal(node.father.data['pos'],node.data['pos']):
                    node_branch.update({node.Idx: node_branch[node.father.Idx]})
                    continue
                branches.append(np.stack((node.father.data['pos'], node.data['pos']), axis=0))
                node_branch.update({node.Idx:bn})

                fabranch=node_branch[node.father.Idx]
                branch_branch.update({bn:fabranch})
                bn = bn+1
                # branch_last.update({node.Idx:bn})
                # father_branch = branch_last[node.father.Idx]
                # branch_start.update({bn:father_branch})
        ...
        for branch in branches :
            offset.append(branch[0])

        for idx, x in enumerate(branches):
            branches[idx] = x - offset[idx]
            branches[idx],max = normalize(branches[idx])
            branches[idx] = resample_branch_by_step(branches[idx], align, len(branches[idx]))

            max_dist.append(max)
            # if branches[idx].shape != (32,3):
            #     print('error')
            #     print(branches[idx].shape)

        return branches,offset,node_branch ,branch_branch,max_dist

    def easy_fetch_noresample(self, align, move=True):
        branches = []
        branch_branch = {}
        branch_last = {}
        node_branch = {}
    ...
    def dfs_prefix_branch(self, curr, father, branches, dataset, layer, node, curr_prefix, neuron_types, neuron_id):
        ...
        if curr.sons is not None:
            all_son = []
            all_leaf=[]
            for cn in range(len(curr.sons)):
                all_son.append(self.dfs_prefix_branch(curr.sons[cn], curr, branches, dataset, layer, node, curr_prefix, neuron_types, neuron_id))
                all_leaf.append(curr.sons[cn].data['leaf'])
            all_son_t=tuple(all_son)
            all_leaf_t=tuple(all_son)

            dataset.append([deepcopy(curr_prefix), all_son_t,all_leaf_t])
...
def grower_process(folder, verbose=False, return_reidx=False, process_folder = None,scaling=1., return_filelist=False):
    neurons = []
    files = sorted([x for x in os.listdir(folder) if not os.path.isdir(x)])
    reidx = {}
    file_list = []
    for f in (files if not verbose else tqdm(files)):
        ...
        file_list.append(f)
        reidx[f] = len(neurons)
        df = smooth(resample(Tree().tree_from_swc(swc, scaling=scaling).cut_node()))
       # df = resample(smooth(Tree().tree_from_swc(swc, scaling=scaling).cut_node()))
        # df = resample(smooth(Tree().tree_from_swc(swc, scaling=scaling)))
        pro_swc = df.to_swc()

        swc_filename = os.path.join(process_folder, f)
        pro_swc.to_csv(swc_filename, sep=' ', index=False, header=False)
