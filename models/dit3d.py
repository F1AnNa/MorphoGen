...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        t = self.t_embedder(t)  #(1,512,384)
        y = self.y_embedder(y, self.training)    
        c = t + y                                

        for block in self.blocks:
            x = block(x, c)                      
        x = self.final_layer(x, c) #(1,512,192)               
        z = x

        x = self.unpatchify_voxels(x)    #(1,3,32,32,32) 

        # Devoxelization
        x = F.trilinear_devoxelize(x, voxel_coords, self.input_size, self.training)#(1,3,2048)
